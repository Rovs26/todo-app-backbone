<template>
  <div class="mb-4">
      ...
      <div class="relative">
        ...
        >
          <svg class="animate-spin h-4 w-4 text-primary-600" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24">
            ...
            <path class="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.374 0 12h4zm2 5.291A7.962 7.962 0 014 12H0c0 3.042 1.135 5.824 3 7.938l3-2.647z"></path>
          </svg>
        </span>
      </div>
    </form>
    ...
    <Transition name="fade">
      <p
        v-if="weatherHint"
        class="mt-1.5 text-xs text-secondary-500 dark:text-secondary-400 flex items-center gap-1"
      >
        {{ weatherHint }}
      </p>
    </Transition>
...
const { load: loadWeather, getHint } = useWeather()

const text = ref('')
const loading = ref(false)
const weatherHint = ref<string | null>(null)

// Debounce timer for weather hints.
let hintTimer: ReturnType<typeof setTimeout> | null = null

function onInput() {
  if (hintTimer) clearTimeout(hintTimer)
  hintTimer = setTimeout(updateHint, 600)
}

async function updateHint() {
  const t = text.value.trim()
  if (!t || t.length < 8) {
    weatherHint.value = null
    return
  }
  // Load weather lazily (cached after first call).
  await loadWeather()
  weatherHint.value = getHint(t)
}
